...
public interface IAgentApi
{
    ...
    Task<Result<IReadOnlyList<ExecutorCandidateDto>>> SendScanExecutorsCommandAsync(Guid agentId, string? executorKey = null, CancellationToken ct = default);
    Task<Result<IReadOnlyList<AgentExecutorConfigDto>>> GetExecutorConfigsAsync(Guid agentId, CancellationToken ct = default);
    Task<Result<IReadOnlyList<Guid>>> GetAgentIdsByExecutorKeyAsync(string executorKey, CancellationToken ct = default);
}
