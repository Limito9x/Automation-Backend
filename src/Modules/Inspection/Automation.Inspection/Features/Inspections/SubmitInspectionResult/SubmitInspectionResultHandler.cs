...
{
    public async Task<Result<InspectionDto>> HandleAsync(SubmitInspectionResultCommand command, CancellationToken ct)
    {
        ...
        if (command.Data.HasValue && command.Data.Value.ValueKind != JsonValueKind.Null && command.Data.Value.ValueKind != JsonValueKind.Undefined)
        {
            var jsonDoc = JsonDocument.Parse(command.Data.Value.GetRawText());
            inspection.Complete(command.Status, jsonDoc, command.ExecutionTimeMs, command.SummaryMessage);
        }
        else
        {
            inspection.Fail(command.SummaryMessage ?? "Inspection finished without data.", command.ExecutionTimeMs);
        }
        ...
        var dto = new InspectionDto(
            ...
            inspection.InspectorVersion?.Version,
